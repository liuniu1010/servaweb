...
public class Role2ChatImpl implements ChatIFC {
    ...
    private Role2ChatImpl() {
    }

    public static Role2ChatImpl getInstance() {
        return new Role2ChatImpl();
    }
    ...
    public String fetchResponse(String session, String userInput) {
        try {
            if(!isEnvironmentReady()) {
                ...
                return (String)dbService.executeSaveTask(new AbsRole2ChatTask() {
                    @Override
    ...
    public void initNewChat(String session) {
        try {
            if(!isEnvironmentReady()) {
                DBServiceIFC dbService = ServiceFactory.getDBService();
                dbService.executeSaveTask(new AbsRole2ChatTask() {
                    @Override
                    public Object save(DBConnectionIFC dbConnection) {
                        ChatIFC chatIFC = super.setupEnvironment(dbConnection);
...
abstract class AbsRole2ChatTask implements DBQueryTaskIFC, DBSaveTaskIFC {
    protected ChatIFC setupEnvironment(DBConnectionIFC dbConnection) {
        OpenAIImpl openAIImpl = Role2AIImpl.getInstance(dbConnection);
        SuperAIIFC superAI = openAIImpl;

        StorageIFC storage = StorageInDBImpl.getInstance(dbConnection);

        Role2ChatImpl chatImpl = Role2ChatImpl.getInstance();
