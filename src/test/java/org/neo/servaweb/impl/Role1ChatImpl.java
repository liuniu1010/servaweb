...
public class Role1ChatImpl implements ChatIFC {
    ...
    public String fetchResponse(String session, String userInput) {
        try {
            if(!isEnvironmentReady()) {
                ...
                return (String)dbService.executeSaveTask(new AbsRole1ChatTask() {
                    @Override
    ...
    public void initNewChat(String session) {
        try {
            if(!isEnvironmentReady()) {
                DBServiceIFC dbService = ServiceFactory.getDBService();
                dbService.executeSaveTask(new AbsRole1ChatTask() {
                    @Override
                    public Object save(DBConnectionIFC dbConnection) {
                        ChatIFC chatIFC = super.setupEnvironment(dbConnection);
